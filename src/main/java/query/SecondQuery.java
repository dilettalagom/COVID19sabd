                ...
                top100RDD.flatMapToPair(new PairFlatMapFunction <
                    ...
                    call(Tuple2<ClassificationWeekYearPojo, GlobalStatisticsPojo> tuplaRDD) throws Exception {
                        ...
                        for (int i=0; i<allInfected.length;i++ ) {
                            ...
                            String weekYear = CalendarUtility.createKeyWeekYear(dateString);
                            ClassificationWeekYearPojo newOne = new ClassificationWeekYearPojo(tuplaRDD._1().getTrendCoefficient(), tuplaRDD._1().getState(),
                                                                                                tuplaRDD._1().getCountry(), tuplaRDD._1().getContinent(),weekYear, dateString);
                            //refactor RDD elements
                            Tuple2<ClassificationWeekYearPojo, Tuple2<String,Double>> temp = new Tuple2<>(newOne,new Tuple2<>( dateString, allInfected[i]));
                            tupleList.add(temp);
                        }
